...
export function computeDashboardState({
  ...
  const setupComplete = hasResume && hasLLM;
...
export function getDashboardPermissions(state: DashboardState): DashboardPermissions {
  switch (state) {
    case 'SETUP_REQUIRED':
      return {
...
export default function AIPrepDashboard({
  ...
  hasResume: propHasResume,
  hasLLM: propHasLLM,
  hasCompletedAssessment: propHasCompletedAssessment,
  onNavigateTab,
  onViewReport,
  embedded = false,
  ...
  const effectiveHasResume = propHasResume !== undefined ? propHasResume : Boolean(setup?.resume_uploaded);
  const effectiveHasLLM = propHasLLM !== undefined ? propHasLLM : Boolean(setup?.api_keys_configured);
  const effectiveHasCompletedAssessment = propHasCompletedAssessment !== undefined 
    ? propHasCompletedAssessment 
    : assessments.some(a => (a.status || '').toUpperCase() === 'COMPLETED');

  const dashboardState = useMemo(() => {
    return computeDashboardState({
      hasResume: effectiveHasResume,
      hasLLM: effectiveHasLLM,
      hasCompletedAssessment: effectiveHasCompletedAssessment,
    });
  }, [effectiveHasResume, effectiveHasLLM, effectiveHasCompletedAssessment]);

  const permissions = useMemo(() => {
    return getDashboardPermissions(dashboardState);
  }, [dashboardState]);

  const isLlmConfigured = effectiveHasLLM;
  const isResumeUploaded = effectiveHasResume;
  const isSetupComplete = effectiveHasLLM && effectiveHasResume;
  ...
  const autoOpenModalCheckedRef = React.useRef(false);
  useEffect(() => {
    if (!loading && !autoOpenModalCheckedRef.current) {
      autoOpenModalCheckedRef.current = true;
      if (dashboardState === 'SETUP_REQUIRED') {
  ...
  const handleDisabledCardClick = useCallback((cardKey: 'start' | 'assessments' | 'analytics' | 'scores') => {
    if (dashboardState === 'SETUP_REQUIRED') {
      let msg = 'Please complete your Resume and LLM setup before accessing assessments.';
      let btnLabel = 'Complete Setup';
      let btnAction = () => {
        setExplanationModal({ isOpen: false, title: '', message: '' });
        setShowSetupModal(true);
      };

      if (!effectiveHasResume && !effectiveHasLLM) {
        msg = 'Please complete your Resume and LLM setup before accessing assessments.';
        btnLabel = 'Complete Setup';
        btnAction = () => {
          setExplanationModal({ isOpen: false, title: '', message: '' });
          setShowSetupModal(true);
        };
      } else if (!effectiveHasResume) {
        msg = 'Please upload your Resume before accessing assessments.';
        btnLabel = 'Upload Resume';
        btnAction = () => {
          setExplanationModal({ isOpen: false, title: '', message: '' });
          handleNavigate('my-resume');
        };
      } else if (!effectiveHasLLM) {
        msg = 'Please configure your LLM API keys before accessing assessments.';
        btnLabel = 'Configure LLM';
        btnAction = () => {
          setExplanationModal({ isOpen: false, title: '', message: '' });
          handleNavigate('my-llm-setup');
        };
      }
    ...
    } else if (dashboardState === 'READY_FOR_ASSESSMENT') {
      setExplanationModal({
        isOpen: true,
        title: 'Complete Your First Assessment',
        message: 'Please start and complete your first assessment to unlock this section.',
        actionButton: {
    ...
    } else if (dashboardState === 'VIEW_ONLY') {
      setExplanationModal({
        isOpen: true,
        title: 'Action Unavailable',
        message: 'You have already completed an assessment. These actions are unavailable because your required setup is incomplete.',
      });
    }
  }, [dashboardState, effectiveHasResume, effectiveHasLLM, onStartAssessment]);

  const handleStartClick = () => {
    if (permissions.canStartAssessment) {
      onStartAssessment();
    } else {
      handleDisabledCardClick('start');
    }
  };
  ...
  return (
    <div className={`relative w-full text-slate-900 dark:text-slate-100 ${embedded ? '' : 'min-h-full bg-slate-50 dark:bg-[#0b0f19] px-4 py-3 sm:px-6 lg:px-8'}`}>
      <div className="mx-auto max-w-6xl space-y-3 sm:space-y-3.5">
        ...
        {view === null ? (
          ...
          <div className="space-y-3 animate-in fade-in duration-150">
            ...
            {dashboardState !== 'VIEW_ONLY' && !isLlmConfigured && !dismissLlmBanner && (
            ...
            {dashboardState !== 'VIEW_ONLY' && !isResumeUploaded && !dismissResumeBanner && (
            ...
            <div className="rounded-3xl border border-gray-200/80 dark:border-gray-800 bg-white dark:bg-gray-900 p-4 sm:p-5 shadow-xs">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-3.5">
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-150 ${!permissions.canStartAssessment
                  ...
                  >
                    {!permissions.canStartAssessment ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className={`text-sm sm:text-base font-bold truncate ${!permissions.canStartAssessment ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors'}`}>
                        Start Assessment
                      </h3>
                      {!permissions.canStartAssessment && (
                    ...
                    <p className={`text-xs sm:text-[13px] mt-0.5 truncate ${!permissions.canStartAssessment ? 'text-slate-400 dark:text-slate-500 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                      {permissions.canStartAssessment
                        ? 'Start a new practice session'
                        : dashboardState === 'VIEW_ONLY'
                          ? 'Unavailable due to incomplete setup'
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-150 ${!permissions.canViewAssessments
                      ? 'bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500'
                      : 'bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 group-hover:bg-gradient-to-br group-hover:from-indigo-900 group-hover:to-purple-400 group-hover:border-transparent group-hover:text-white dark:group-hover:text-white group-hover:shadow-md group-hover:scale-105'
                      }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className={`text-sm sm:text-base font-bold truncate ${!permissions.canViewAssessments ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors'}`}>
                        View Assessments
                      </h3>
                      {!permissions.canViewAssessments && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 shrink-0">
                    ...
                    <p className={`text-xs sm:text-[13px] mt-0.5 truncate ${!permissions.canViewAssessments ? 'text-slate-400 dark:text-slate-500 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                      {permissions.canViewAssessments
                        ? 'Review all practice sessions'
                        : 'Complete first assessment to unlock'}
                    </p>
                  </div>
                </div>
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-150 ${!permissions.canViewAnalytics
                      ? 'bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500'
                      : 'bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 group-hover:bg-gradient-to-br group-hover:from-indigo-900 group-hover:to-purple-400 group-hover:border-transparent group-hover:text-white dark:group-hover:text-white group-hover:shadow-md group-hover:scale-105'
                      }`}
                  >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className={`text-sm sm:text-base font-bold truncate ${!permissions.canViewAnalytics ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors'}`}>
                        Analytics
                      </h3>
                      {!permissions.canViewAnalytics && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 shrink-0">
                    ...
                    <p className={`text-xs sm:text-[13px] mt-0.5 truncate ${!permissions.canViewAnalytics ? 'text-slate-400 dark:text-slate-500 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                      {permissions.canViewAnalytics
                ...
                >
                  <div
                    className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 transition-all duration-150 ${!permissions.canViewScores
                      ? 'bg-gray-100 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-500'
                      : 'bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-700 text-gray-600 dark:text-gray-300 group-hover:bg-gradient-to-br group-hover:from-indigo-900 group-hover:to-purple-400 group-hover:border-transparent group-hover:text-white dark:group-hover:text-white group-hover:shadow-md group-hover:scale-105'
                      }`}
                  >
                    {!permissions.canViewScores ? (
                      <LockKeyhole className="w-4 h-4" />
                    ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className={`text-sm sm:text-base font-bold truncate ${!permissions.canViewScores ? 'text-slate-500 dark:text-slate-400' : 'text-slate-900 dark:text-white group-hover:text-purple-700 dark:group-hover:text-purple-300 transition-colors'}`}>
                        Scores
                      </h3>
                      {!permissions.canViewScores && (
                        <span className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded-full text-[10px] font-bold bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400 border border-gray-200 dark:border-gray-700 shrink-0">
                    ...
                    <p className={`text-xs sm:text-[13px] mt-0.5 truncate ${!permissions.canViewScores ? 'text-slate-400 dark:text-slate-500 font-medium' : 'text-slate-500 dark:text-slate-400'}`}>
                      {permissions.canViewScores
                      ...
                      <div className="flex items-center gap-2.5">
                        ...
                        <div>
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-semibold border ${selItem.status === 'COMPLETED'
                            ...
                            >
                              <span
                                className={`w-1.5 h-1.5 rounded-full ${selItem.status === 'COMPLETED'
                                    ? 'bg-emerald-500'
                                    : selItem.status === 'IN_PROGRESS' || selItem.status === 'EVALUATING'
                                      ? 'bg-amber-500'
                                      : 'bg-slate-400'
                      ...
                      <div className="rounded-xl border border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800/60 p-3">
                        ...
                        <div className="mt-1 flex items-baseline gap-1.5">
                          {loadingScoreDetails[selItem.id] ? (
                            ...
                            <p
                              className={`text-base sm:text-lg font-black ${selScore != null
                        ...
                        return (
                          ...
                          >
                            <td className="py-3 px-4">
                              <div
                                className={`flex h-7 w-7 items-center justify-center rounded-lg font-bold text-xs ${isSelected
                            ...
                            <td className="py-3 px-4">
                              <span
                                className={`inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[10px] sm:text-[11px] font-semibold border ${item.status === 'COMPLETED'
                              ...
                              >
                                <span
                                  className={`w-1.5 h-1.5 rounded-full ${item.status === 'COMPLETED'
                                      ? 'bg-emerald-500'
                                      : item.status === 'IN_PROGRESS' || item.status === 'EVALUATING'
                                        ? 'bg-amber-500'
                                        : 'bg-slate-400'
                  ...
                  filteredScoreAssessments.map((item) => {
                    ...
                    return (
                      <div
                        key={item.id}
                        onClick={() => handleSelectScoreAssessment(item)}
                        className={`rounded-2xl border p-3.5 transition-all cursor-pointer ${isSelected
                      ...
                      >
                        ...
                        <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-slate-200/50 dark:border-slate-700/50 text-xs">
                          <div className="flex items-center gap-2">
                            ...
                            <span
                              className={`px-2 py-0.5 rounded-md text-[10px] font-semibold ${item.status === 'COMPLETED'
      ...
      {/* Contextual Explanation Modal for Disabled Cards */}
      {typeof document !== 'undefined' && explanationModal.isOpen && createPortal(
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/60 p-4 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 sm:p-7 shadow-2xl dark:bg-slate-900 border border-slate-200 dark:border-slate-800">
            <div className="flex items-center justify-between">
              ...
              <button
                type="button"
                onClick={() => setExplanationModal({ isOpen: false, title: '', message: '' })}
                className="rounded-lg p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:hover:bg-slate-800 dark:hover:text-slate-200 transition-colors cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            <p className="mt-2 text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed">
              {explanationModal.message}
            </p>

            <div className="mt-6 flex flex-col gap-2.5">
              {explanationModal.actionButton && (
                <button
                  type="button"
                  onClick={explanationModal.actionButton.onClick}
                  className="w-full rounded-xl bg-gradient-to-r from-indigo-900 to-purple-600 px-4 py-2.5 text-xs font-bold text-white hover:from-indigo-800 hover:to-purple-500 transition-all cursor-pointer shadow-sm flex items-center justify-center gap-2"
                >
                  <span>{explanationModal.actionButton.label}</span>
                  <ArrowRight className="h-4 w-4" />
                </button>
              )}
              <button
