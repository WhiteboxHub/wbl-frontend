...
export const DeviceCheckWizard: React.FC<DeviceCheckWizardProps> = ({
  ...
  useEffect(() => {
    if (!videoEnabled || !cameraStream || !isVisionReady) return;
  ...
  }, [videoEnabled, cameraStream, isVisionReady, detectVideoFrame]);

  // Monitor live telemetry emissions to track instantaneous real-time face presence
  useEffect(() => {
    if (realtimeTelemetry && Object.keys(realtimeTelemetry).length > 0 && (realtimeTelemetry.face_box || realtimeTelemetry.is_instant_face_present)) {
      lastTelemetryUpdateRef.current = Date.now();
      setIsFaceLive(true);
    }
  }, [realtimeTelemetry]);

  // Fast Watchdog: clears face live state within 1200ms if candidate leaves frame or camera is blocked
  useEffect(() => {
    const interval = setInterval(() => {
      if (lastTelemetryUpdateRef.current > 0 && Date.now() - lastTelemetryUpdateRef.current > 1200) {
        setIsFaceLive(false);
      }
    }, 250);
  ...
  // 🔍 PRE-CHECK DIAGNOSTICS MODULE (Face Detection, Position, Lighting, Focus)
  // ============================================================================
  // Vision Telemetry status metrics with strict real-time instantaneous evaluation
  ...
  const isEyesOnScreen = isFaceDetected && (realtimeTelemetry?.is_instant_eyes_attentive ?? false);
  // Strict user requirement: focus should be optimal ONLY when face is in frame AND eyes are looking towards screen
  const isFocusOptimal = isFaceDetected && isCentered && isEyesOnScreen;
                  ...
                  <div className={`col-span-1 lg:col-span-7 xl:col-span-8 flex flex-col ${videoEnabled ? 'justify-between h-full space-y-2' : 'space-y-2 sm:space-y-2.5'}`}>
                    ...
                    <div className={`relative w-full rounded-2xl overflow-hidden shadow-xs border border-slate-200 dark:border-slate-800 flex items-center justify-center ${!videoEnabled ? (((micTested && micOk === false) || (internetStatus === 'unstable' || internetStatus === 'failed') || (speakerTested && speakerOk === false)) ? 'min-h-[185px] sm:min-h-[205px] max-h-[230px]' : 'min-h-[285px] sm:min-h-[315px] lg:min-h-[335px] max-h-[380px]') + ' bg-[#F7F9FE] dark:bg-slate-900/90' : (((micTested && micOk === false) || (cameraTested && cameraOk === false) || (internetStatus === 'unstable' || internetStatus === 'failed')) ? 'aspect-[16/9] min-h-[160px] sm:min-h-[185px] max-h-[210px]' : 'aspect-[16/9] min-h-[240px] sm:min-h-[275px] lg:min-h-[295px] max-h-[320px]') + ' bg-slate-950'}`}>
                      {!videoEnabled ? (
                        ...
                        <>
                          ...
                          />
                          {cameraOk && (
                            <div className="absolute top-3 left-3 z-10 flex items-center gap-2 px-3 py-1 rounded-full bg-black/70 backdrop-blur-md border border-white/15 text-[11px] font-medium text-white shadow-sm">
                              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                              <span>Camera preview &bull; 640x480 @ 30fps</span>
                            </div>
                          )}
                          {/* ============================================================================ */}
                          {/* 🔍 PRE-CHECK DIAGNOSTICS MODULE (Face Detection, Position, Lighting, Focus) */}
                          ...
                          {cameraOk && (
                            <div className="absolute inset-0 z-20 flex items-center justify-center pointer-events-none">
                              {isFaceDetected && isCentered ? (
                                /* 1. When Face is Aligned and Centered: Solid Green Frame with Position Optimal */
                                <div className="relative w-[220px] sm:w-[250px] lg:w-[270px] h-[240px] sm:h-[270px] lg:h-[300px] border-2 border-emerald-400 bg-emerald-500/5 rounded-3xl shadow-[0_0_20px_rgba(52,211,153,0.25)] flex items-center justify-center transition-all duration-200">
                                  {/* 4 Corner Markers */}
                                  <div className="absolute -top-1 -left-1 w-3.5 h-3.5 border-t-2 border-l-2 border-emerald-400 rounded-tl-lg" />
                                  <div className="absolute -top-1 -right-1 w-3.5 h-3.5 border-t-2 border-r-2 border-emerald-400 rounded-tr-lg" />
                                  <div className="absolute -bottom-1 -left-1 w-3.5 h-3.5 border-b-2 border-l-2 border-emerald-400 rounded-bl-lg" />
                                  <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 border-b-2 border-r-2 border-emerald-400 rounded-br-lg" />

                                  {/* Floating Green Pill Tag */}
                                  <div className="absolute -top-3.5 left-1/2 -translate-x-1/2 flex items-center gap-1.5 px-3 py-0.5 rounded-full bg-slate-950/85 backdrop-blur-md border border-emerald-400/60 text-[10.5px] font-bold text-emerald-400 shadow-md whitespace-nowrap">
                                    <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-ping" />
                                    <span>Position Optimal</span>
                                  </div>
                                </div>
                              ) : (
                                /* 2. When Face is NOT Aligned or Searching: Dashed Amber Frame with Align face in frame */
                                <div className="relative w-[220px] sm:w-[250px] lg:w-[270px] h-[240px] sm:h-[270px] lg:h-[300px] border-2 border-dashed border-amber-400/80 bg-amber-500/5 rounded-3xl shadow-[0_0_15px_rgba(245,158,11,0.15)] flex items-center justify-center transition-all duration-200">
                                  {/* 4 Corner Accents */}
                                  <div className="absolute -top-1 -left-1 w-3.5 h-3.5 border-t-2 border-l-2 border-amber-400 rounded-tl-lg" />
                                  <div className="absolute -top-1 -right-1 w-3.5 h-3.5 border-t-2 border-r-2 border-amber-400 rounded-tr-lg" />
                                  <div className="absolute -bottom-1 -left-1 w-3.5 h-3.5 border-b-2 border-l-2 border-amber-400 rounded-bl-lg" />
                                  <div className="absolute -bottom-1 -right-1 w-3.5 h-3.5 border-b-2 border-r-2 border-amber-400 rounded-br-lg" />

                                  {/* Center Amber Tag */}
                                  <div className="px-3 py-1.5 rounded-lg bg-black/80 backdrop-blur-md border border-amber-400/60 text-[11px] font-bold text-amber-300 flex items-center gap-1.5 shadow-md">
                    ...
                    {videoEnabled && cameraOk && (
                      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 w-full">
                        {/* 1. Face Detection */}
                        <div className="flex items-center justify-between px-3.5 py-2.5 rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                          <div className="flex items-center gap-2 min-w-0">
                            <User className="w-4 h-4 text-slate-600 dark:text-slate-400 shrink-0" />
                            <span className="text-xs sm:text-[13px] font-semibold text-slate-800 dark:text-slate-200 truncate">Face Detection</span>
                          </div>
                          {isFaceDetected ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                              Detected
                            </span>
                          ) : (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-rose-50 text-rose-600 dark:bg-rose-950/50 dark:text-rose-400 border border-rose-200 dark:border-rose-800/60 shrink-0">
                        ...
                        <div className="flex items-center justify-between px-3.5 py-2.5 rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                          <div className="flex items-center gap-2 min-w-0">
                            <Scan className="w-4 h-4 text-slate-600 dark:text-slate-400 shrink-0" />
                            <span className="text-xs sm:text-[13px] font-semibold text-slate-800 dark:text-slate-200 truncate">Position</span>
                          </div>
                          {isCentered ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                            ...
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-amber-50 text-amber-600 dark:bg-amber-950/50 dark:text-amber-400 border border-amber-200 dark:border-amber-800/60 shrink-0">
                        ...
                        <div className="flex items-center justify-between px-3.5 py-2.5 rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                          <div className="flex items-center gap-2 min-w-0">
                            <Sun className="w-4 h-4 text-slate-600 dark:text-slate-400 shrink-0" />
                            <span className="text-xs sm:text-[13px] font-semibold text-slate-800 dark:text-slate-200 truncate">Lighting</span>
                          </div>
                          {hasGoodLighting ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                        ...
                        <div className="flex items-center justify-between px-3.5 py-2.5 rounded-2xl border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xs">
                          <div className="flex items-center gap-2 min-w-0">
                            <Eye className="w-4 h-4 text-slate-600 dark:text-slate-400 shrink-0" />
                            <span className="text-xs sm:text-[13px] font-semibold text-slate-800 dark:text-slate-200 truncate">Focus</span>
                          </div>
                          {isFocusOptimal ? (
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[11px] font-semibold bg-emerald-50 text-emerald-600 dark:bg-emerald-950/50 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800/60 shrink-0">
                              <span className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
