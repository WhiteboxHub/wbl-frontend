...
export default function AssessmentCard({
  ...
  useEffect(() => {
    const loadAssessment = async () => {
      try {
        if (assessmentId === "new") {
          // 1. Create a new assessment session via the AI Prep backend.
          const created = await aiPrepApi.createAssessment();
          // 2. Use the real assessment ID returned by the backend.
  ...
  // Use the backend-returned assessment ID for navigation.
  const resolvedId = assessment?.id ?? assessmentId;

  return (
    <section className="bg-slate-50 py-4">
      <div className="mx-auto max-w-3xl">
        {/* Back button – navigates to /user_dashboard/aiprep */}
        <button
          onClick={() =>
        ...
        <div className="mt-5 rounded-2xl border border-slate-200 bg-white p-7 shadow-sm">
          ...
          {/* Navigate to the report using the real backend assessment ID */}
          <button
