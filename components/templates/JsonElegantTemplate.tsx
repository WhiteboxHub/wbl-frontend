...
export function JsonElegantTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="bg-white text-gray-800 min-h-[842px] font-sans p-10 text-[13px]">
          ...
          <section>
            ...
            {data.education.map((edu) => (
              <div key={edu.id} className="mb-2">
                ...
                <div className="text-gray-400 text-xs">{edu.startDate}{edu.endDate ? ` – ${edu.endDate}` : ""}</div>
              </div>
            ))}
          </section>
        )}
          ...
          <section>
            ...
            <div className="flex flex-wrap gap-1.5">
              {data.skills.filter(Boolean).map((s, i) => (
                <span key={i} className="border border-gray-300 text-gray-600 px-2 py-0.5 rounded text-xs">{s}</span>
              ))}
            </div>
          </section>
        )}
