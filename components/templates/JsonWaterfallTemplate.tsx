...
export function JsonWaterfallTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="bg-white text-gray-800 min-h-[842px] font-sans text-[13px]">
      {/* Top cascade */}
              ...
              {data.experience.map((exp, idx) => (
                <div key={exp.id} className="relative pl-4 border-l-2" style={{ borderColor: idx % 2 === 0 ? "#0f3460" : "#e94560" }}>
                  ...
                  <div className="flex justify-between items-baseline">
                    ...
                    <span className="text-xs text-gray-400">{formatDateRange(exp.startDate, exp.endDate, exp.current)}</span>
                  </div>
                  <div className="text-[#0f3460] text-[13px] font-medium">{exp.title}{exp.location ? ` · ${exp.location}` : ""}</div>
            ...
            <section>
              ...
              <div className="flex flex-wrap gap-1.5 pl-4">
                {data.skills.filter(Boolean).map((s, i) => (
                  <span key={i} className="bg-[#1a1a2e] text-white px-2 py-0.5 rounded text-[11px]">{s}</span>
                ))}
              </div>
            </section>
          )}
