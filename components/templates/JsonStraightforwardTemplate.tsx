...
export function JsonStraightforwardTemplate({ data }: TemplateProps) {
  ...
  return (
    <div className="bg-white text-gray-900 min-h-[842px] font-mono p-10 text-[12.5px]">
            ...
            {data.experience.map((exp) => (
              <div key={exp.id}>
                <div className="flex flex-wrap justify-between items-baseline gap-1">
                  <strong className="text-[13px]">{exp.company} — {exp.title}</strong>
          ...
          <section>
            ...
            {data.education.map((edu) => (
              <div key={edu.id} className="mb-2">
                <strong className="text-[12.5px]">{edu.degree}, {edu.field}</strong>
