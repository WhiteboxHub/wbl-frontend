          ...
          <section className="mb-6">
            {data.experience.map((exp, idx) => (
              <div key={exp.id} className="flex gap-8 mb-5 pb-5" style={{ borderBottom: idx < data.experience.length - 1 ? '1px solid #f3f4f6' : 'none' }}>
                <div className="w-24 shrink-0 text-right text-xs text-gray-400 pt-0.5">
                ...
                <div className="flex-1">
                  ...
                  <div className="text-gray-600 text-[13px]">{exp.title}{exp.location ? ` · ${exp.location}` : ""}</div>
          ...
          <section className="flex gap-8">
            ...
            <div className="flex-1 flex flex-wrap gap-1.5">
              {data.skills.filter(Boolean).map((s, i) => (
                <span key={i} className="text-gray-700 text-xs bg-gray-100 px-2 py-0.5 rounded">{s}</span>
              ))}
            </div>
          </section>
        )}
