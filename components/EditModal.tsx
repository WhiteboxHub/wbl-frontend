...
// //                                   <select
// //                                     {...register(key)}
// //                                     value={currentFormValues[key] || formData[key] || "0"}
// //                                     className="w-full px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white hover:border-blue-300 transition shadow-sm"
// //                                   >
// //                                     <option value="0">0</option>
// //                                     {subjects.map((subject) => (
...
// //                                     className="w-full px-2 sm:px-3 py-1.5 sm:py-2 text-xs sm:text-sm border border-blue-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-400 bg-white hover:border-blue-300 transition shadow-sm"
// //                                   >
// //                                     {reorderYesNoOptions(key, currentValue, enumOptions[key.toLowerCase()]).map((opt) => (
// //                                       <option key={opt.value} value={opt.value}>
// //                                         {opt.label}
// //                                       </option>
// //                                     ))}
...
// //     // Exclude material_type from non-course material modals
// //     if (!isCourseMaterialModal && key.toLowerCase() === "material_type") {
// //       return;
// //     }
...
// //                         {/* Course Material Specific Dropdowns - Only show for course materials */}
// //                         {isCourseMaterialModal && section === "Professional Information" && (
// //                           <div className="space-y-1 sm:space-y-1.5">
// //                             <label className="block text-xs sm:text-sm font-bold text-blue-700">
// //                               Course Name
...
// //                             {/* Material Type Dropdown - ONLY for Course Materials */}
// //                             <div className="space-y-1 sm:space-y-1.5">
// //                               <label className="block text-xs sm:text-sm font-bold text-blue-700">
// //                                 Material Type
// //                               </label>
...
// // Vendor-specific options for the `type` field
...
// // Material-specific options for the `type` field (used in course materials)
...
// // Other enum options (e.g., yes/no, status, etc.)
// const enumOptions: Record<string, { value: string; label: string }[]> = {
//   move_to_prep: [
//     { value: "false", label: "No" },
//     { value: "true", label: "Yes" },
...
//                             const isTypeField = key.toLowerCase() === "type";
//                             const isBatchField = key.toLowerCase() === "batchid";
//                             const isVendorStatusField = key.toLowerCase() === "status" && isVendorModal;
//                             const isMaterialTypeField = key.toLowerCase() === "material_type";
...
//                             // For all other type fields, render as text input or dropdown based on enum
//                             if (isTypeField) {
//                               if (enumOptions[key.toLowerCase()]) {
//                                 const currentValue = currentFormValues[key] || formData[key] || "";
//                                 return (
...
// Vendor-specific status options
const vendorStatuses = [
  { value: "active", label: "Active" },
  { value: "working", label: "Working" },
  { value: "not_useful", label: "Not Useful" },
...
// Generic status options (for non-vendor modals)
    ...
    <>
      {isOpen && (
        <div className="fixed inset-0 bg-black bg-opacity-30 flex items-center justify-center p-2 sm:p-4 z-50">
          ...
          >
            {/* Modal Header */}
            <div className="sticky top-0 bg-gradient-to-r from-blue-50 via-purple-50 to-pink-50 px-3 sm:px-4 md:px-6 py-2.0 sm:py-2  border-b border-blue-200 flex justify-between items-center">
              <h2 className="text-sm sm:text-base md:text-lg font-semibold bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent">
                {title} - Edit Details
              </h2>
              <button
                  ...
                  {visibleSections
                    ...
                    .map((section) => (
                      <div key={section} className="space-y-3 sm:space-y-4">
                        ...
                        {/* Course-Subject Specific Dropdowns - Course Name in Professional Information */}
                        ...
                        {/* Course Material Specific Dropdowns - cm_subject in Basic Information */}
                        {isCourseMaterialModal && section === "Basic Information" && (
                          <div className="space-y-1 sm:space-y-1.5">
                            <label className="block text-xs sm:text-sm font-bold text-blue-700">
                              Subject Name
                        ...
                        {/* Course-Subject Specific Dropdowns - Subject Name in Basic Information */}
                        ...
                        {sectionedFields[section]
                          ...
                          .map(({ key, value }) => {
                            ...
                            // Show generic status dropdown for non-vendor modals
                            if (isStatusField && !isVendorModal) {
                              return (
                                <div key={key} className="space-y-1 sm:space-y-1.5">
                                  <label className="block text-xs sm:text-sm font-bold text-blue-700">
