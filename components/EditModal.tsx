...
export function EditModal({
  ...
  // Fixed: Moved state initialization before any function calls
  const [allBatches, setAllBatches] = useState<Batch[]>([]);
  const [mlBatches, setMlBatches] = useState<Batch[]>([]);
  const [batchesLoading, setBatchesLoading] = useState(true);
                      ...
                      <div key={key} className="space-y-1">
                        ...
                        {key.toLowerCase() === "courseid" ? (
                          ...
                          <select
                            value={formData.batchid || ""}
                            onChange={(e) => {
                              const selectedBatch = mlBatches.find(batch => batch.batchid === Number(e.target.value));
                              handleChange("batchid", Number(e.target.value));
                              if (selectedBatch) {
                                handleChange("batchname", selectedBatch.batchname);
                              }
                          ...
                          >
                            <option value="">
                              {batchesLoading ? "Loading batches..." : "Select a batch (optional)"}
                            </option>
                            {mlBatches.map(batch => (
                              <option key={batch.batchid} value={batch.batchid}>
                                {batch.batchname}
