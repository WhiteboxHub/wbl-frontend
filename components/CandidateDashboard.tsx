...
type TabType = 'overview' | 'my-sessions' | 'my-interviews' | 'job-board' | 'wbl-smartprep' | 'ai-prep' | 'my-llm-key' | 'my-applications' | 'my-llm-setup' | 'my-resume';

const normalizeTab = (tab?: string): TabType => {
    if (!tab) return 'overview';
    if (tab.startsWith('ai-prep') || tab.startsWith('wbl-smartprep')) return 'ai-prep';
    return tab as TabType;
};

const WIZARD_SLUGS = ['assesment-type', 'assessment-type', 'device-check', 'devicecheck', 'consent', 'practice', 'practice-start', 'configuration'];
    ...
    useEffect(() => {
        const handlePopState = () => {
            if (typeof window !== "undefined") {
                ...
                const tabFromUrl = pathSegments.slice(1).join("/");
                if (tabFromUrl) {
    ...
    const loadDashboard = useCallback(async (retryCount = 0) => {
        ...
        try {
            ...
            const requiredFields = [
                ...
                'dob', 'github_link', 'address',
                'linkedin_id', 'education', 'zip_code'
            ];
            ...
            try {
                ...
                if (timeData?.server_time) {
                    const serverTimeUtc = /Z|[+-]\d{2}(:\d{2})?$/.test(timeData.server_time) ? timeData.server_time : timeData.server_time + 'Z';
                    serverTimeStr = serverTimeUtc;
                    ...
                    <div>
                        ...
                        <div className="space-y-0.5">
                            ...
                            <button
                                ...
                                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-semibold transition-all duration-150 ${activeTab === 'ai-prep' || activeTab === 'wbl-smartprep'
                                    ? "bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400"
                                    : "text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-gray-800/60 hover:text-gray-900 dark:hover:text-white"
                                    }`}
                            >
                                <Sparkles className={`w-4 h-4 flex-shrink-0 ${activeTab === 'ai-prep' || activeTab === 'wbl-smartprep' ? "text-indigo-600 dark:text-indigo-400" : "text-gray-400"}`} />
                                <span>AI PrepTool</span>
                                {(activeTab === 'ai-prep' || activeTab === 'wbl-smartprep') && <div className="ml-auto w-1.5 h-1.5 rounded-full bg-indigo-500" />}
                            </button>
                        </div>
                    </div>
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col animate-fadeIn">
                        {setupWizardOpen ? (
                            ...
                            <>
                                ...
                                {(activeTab === 'ai-prep' || activeTab === 'wbl-smartprep') && (
