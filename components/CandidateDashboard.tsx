...
export default function CandidateDashboard({ defaultTab = 'overview' }: CandidateDashboardProps) {
    ...
    const checkCandidateAssessments = useCallback(async (cId: number) => {
        try {
            const res = await aiprepApi.listCandidateAssessments(cId);
            const hasCompleted = (res?.items || []).some((item: any) => item.status === 'COMPLETED');
            setHasCompletedAssessment(hasCompleted);
        } catch (e) {
            console.warn("Could not check candidate assessments:", e);
        }
    }, []);

    useEffect(() => {
        if (candidateId) {
            void checkCandidateAssessments(candidateId);
        }
    }, [candidateId, checkCandidateAssessments]);

    useEffect(() => {
        if (activeTab === 'wbl-smartprep' && candidateId) {
            void checkCandidateAssessments(candidateId);
        }
    }, [activeTab, candidateId, checkCandidateAssessments]);
