...
//   const [sessionTypes, setSessionTypes] = useState<SessionType[]>([]);
//   const [sessions, setSessions] = useState<Session[]>([]);
//   const [selectedType, setSelectedType] = useState("Group Mock");
//   const [selectedSession, setSelectedSession] = useState<Session | null>(null);
//   const [error, setError] = useState<string | null>(null);
...
//       })
//       .then((data) => {
//         // Convert string array from backend to objects for sorting
//         const typesArray = data.types.map((t: string) => ({ type: t }));

//         // Sort with fallback for unknown types
//         const sortedTypes = typesArray.sort((a, b) => {
...
//         setError("Error fetching session types.");
//       })
//       .finally(() => setIsLoadingSessionTypes(false));
//   }, []);
...
//   // Fetch sessions whenever course or type changes
//   useEffect(() => {
//     setSessions([]);
//     setSelectedSession(null);
//     setError(null);
...
// export default SessionComp;
  ...
  //  above data is showing but video not palying 
...
// const sortSessionTypes = (types: SessionType[]) => {
//   return types.sort((a, b) => {
//     if (a.type === "Internal Sessions") return 1;
//     if (b.type === "Internal Sessions") return -1;
//     return desiredOrder.indexOf(a.type) - desiredOrder.indexOf(b.type);
//   });
// };
...
const desiredOrder = [
  ...
  "Job Help",
  "Misc",
  "Internal Sessions", // ✅ consistent label in UI
];
...
const sortSessionTypes = (types: SessionType[]) => {
  return types
    .map((t) => ({ ...t, type: normalizeType(t.type) })) // normalize before sorting
    .sort((a, b) => desiredOrder.indexOf(a.type) - desiredOrder.indexOf(b.type));
};
...
const SessionComp = () => {
  ...
  const fetchSessions = () => {
    ...
    if (selectedType === "Internal Sessions") {
      apiType = "Internal Session"; // backend might expect this
    }
