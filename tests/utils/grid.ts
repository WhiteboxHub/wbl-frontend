...
export async function validateAllTables(
  ...
  if (expectGrid) {
    console.log(
      "[grid] Grid expected — waiting up to 3 min for it to appear...",
    );
    try {
      await gridWrapper.first().waitFor({ state: "visible", timeout: 180_000 });
    } catch {
      throw new Error(
        `[grid] FAIL: A grid was expected on route "${page.url()}" but none appeared within 3 min. ` +
  ...
  for (let gridIndex = 0; gridIndex < gridCount; gridIndex++) {
    ...
    if (await loadingOverlay.isVisible()) {
      console.log(
        `[grid] ${gridLabel}: Loading spinner visible — waiting up to 3 min for data...`,
      );
      await loadingOverlay.waitFor({ state: "hidden", timeout: 180_000 });
