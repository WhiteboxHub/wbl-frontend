...
export default function CourseMaterialPage() {
  ...
  // ==========================================
  // Helper Functions - DISPLAY NAMES ONLY
  // ==========================================
  const getSubjectDisplayName = (subjectId: number) => {
    if (subjectId === 0) return "Basic Fundamentals";
    const subject = subjects.find((s) => s.id === subjectId);
  ...
  // ==========================================
  // Column Definitions - DISPLAY NAMES, SEND IDs
  // ==========================================
  const columnDefs: ColDef[] = useMemo<ColDef[]>(() => [
    { 
      field: "id", 
    ...
    },
    {
      ...
      width: 180,
      editable: true,
      // DISPLAY: Show the subject name in the grid
      valueFormatter: (params) => {
        return getSubjectDisplayName(params.value);
      },
      // EDIT: Use dropdown with IDs as values but show names
      cellEditor: "agRichSelectCellEditor",
      cellEditorParams: {
        values: [
          0,  // Basic Fundamentals
          ...subjects.map(s => s.id)  // All subject IDs
        ],
        formatValue: (value: number) => {
          // This shows the NAME in the dropdown
    ...
    },
    {
      ...
      width: 180,
      editable: true,
      // DISPLAY: Show the course name in the grid
      valueFormatter: (params) => {
      ...
      cellEditorParams: {
        values: [
          0,  // Fundamentals
          ...courses.map(c => c.id)  // All course IDs
        ],
        formatValue: (value: number) => {
          // This shows the NAME in the dropdown
    ...
    },
    {
      ...
      width: 150,
      editable: true,
      // DISPLAY: Show full type name (e.g., "Presentations")
      valueFormatter: (params) => {
        return getTypeDisplayName(params.value);
      },
      // EDIT: Use dropdown with codes as values but show names
      cellEditor: "agRichSelectCellEditor",
      cellEditorParams: {
        values: Object.keys(TYPE_MAPPING),  // ["P", "C", "D", ...]
        formatValue: (value: string) => {
          // This shows the FULL NAME in the dropdown
  ...
  const handleRowUpdated = async (updatedRow: any) => {
    try {
      // ✅ The row data already contains IDs (not names) because of our column config
      const payload = {
        subjectid: Number(updatedRow.subjectid),  // ✅ This is already an ID
        courseid: Number(updatedRow.courseid),    // ✅ This is already an ID
        name: updatedRow.name,
        description: updatedRow.description,
        type: updatedRow.type,                     // ✅ This is already a code (P, C, D, etc.)
        link: updatedRow.link,
        sortorder: Number(updatedRow.sortorder)
      };
