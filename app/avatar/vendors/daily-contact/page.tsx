...
export default function VendorContactsGrid() {
  ...
  const handleRowUpdated = async (updatedData: any) => {
    try {
      ...
      if (processedData.moved_to_vendor !== undefined) {
        processedData.moved_to_vendor = parseMovedToVendor(
          processedData.moved_to_vendor
        );
      }
  ...
  const handleMoveAll = async () => {
    ...
    try {
      ...
      toast.success(`${response.updated_count} contacts moved`);
      fetchContacts();
    } catch (error) {
