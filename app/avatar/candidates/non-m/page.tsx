  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const marketingIds = new Set(mktCandidates.map((m: any) => m.candidate_id));
        const nonMarketing = prepCandidates.filter((p: any) => !marketingIds.has(p.candidate_id));
      ...
      <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
        <div>
          <h1 className="text-2xl font-bold text-gray-900 dark:text-gray-100">
            Non-Marketing Candidates
          </h1>
          <p className="mt-1 text-gray-600 dark:text-gray-400">
            Candidates in Prep who are not yet in Marketing.
          </p>
        </div>
      </div>
