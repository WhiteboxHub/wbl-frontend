...
// import { SearchIcon, User, Phone, DollarSign, BookOpen, Briefcase, Mail, Eye } from "lucide-react";
...
// const DateFormatter = (date: any) => (date ? new Date(date).toLocaleDateString() : "Not Set");
// const AmountFormatter = (amount: any) => (amount ? `$${Number(amount).toLocaleString()}` : "Not Set");
...
//   if (/^[a-z0-9.-]+\.(com|org|net|io|co)(\/.*)?$/i.test(trimmed)) return true;
//   return false;
// }
...
//   return (
//     <button
//       className="bg-blue-500 hover:bg-blue-600 text-white px-2 py-1 rounded text-xs font-medium transition-colors cursor-pointer"
//       onClick={(e) => {
//         e.preventDefault();
//         e.stopPropagation();
...
//   // 🔹 Fetch candidate details (always with token)
//   const fetchCandidateById = async (id: number) => {
//     setLoading(true);
//     try {
//       const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/details/${id}`, {
//         headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
//       });
//       const data = await res.json();
//       const sessionRes = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/sessions/${id}`, {
//         headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
//       });
//       const sessionData = await sessionRes.json();
//       setSelectedCandidate({ ...data, session_records: sessionData.sessions || [] });
//     } catch (err) {
...
//   const selectCandidate = (s: CandidateSuggestion) => fetchCandidateById(s.id);

//   useEffect(() => {
//     if (candidateIdFromUrl) {
//       if (!selectedCandidate || selectedCandidate.candidate_id !== Number(candidateIdFromUrl)) {
...
//           <h2>{selectedCandidate.basic_info?.full_name}</h2>
//           <p>Email: {selectedCandidate.basic_info?.email}</p>
//         </div>
//       )}
//     </div>
//   );
...
// Helper: Check if a value is a potential URL
function isPotentialUrl(val: any): boolean {
  if (typeof val !== "string") return false;
  const trimmed = val.trim();
...
// Helper: Render a button to open a link in a new tab
function renderOpenLinkButton(url: string, label: string = "Open") {
  let finalUrl = url.trim();
...
export default function CandidateSearchPage() {
  ...
  const selectCandidate = async (suggestion: CandidateSuggestion) => {
    ...
    setSearchTerm("");
    try {
      const [detailsRes, sessionRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/details/${suggestion.id}`, {
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/sessions/${suggestion.id}`, {
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
        })
      ]);

      if (!detailsRes.ok) throw new Error('Failed to fetch candidate details');
  ...
  const fetchCandidateById = async (id: number) => {
    ...
    try {
      const [detailsRes, sessionRes] = await Promise.all([
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/details/${id}`, {
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
        }),
        fetch(`${process.env.NEXT_PUBLIC_API_URL}/candidates/sessions/${id}`, {
          headers: { ...getAuthHeaders(), 'Content-Type': 'application/json' }
        })
      ]);
    ...
    <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-6 shadow-sm">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-1 text-sm">
        {Object.entries(data).map(([key, value]) => {
          ...
          if ((key === 'Last Login' && value) || (key === 'Last Modified' && value)) {
          ...
          } else if (key === 'candidate_folder' && value && value.toString().trim() !== '') {
            const url = value.toString().trim();
            let finalUrl = url;
            if (!url.startsWith('http://') && !url.startsWith('https://')) {
              finalUrl = 'https://' + url;
            ...
            displayValue = (
              <div className="flex flex-wrap gap-1">
                {value.map((url: string, idx: number) => (
                  <span key={idx}>{renderOpenLinkButton(url, `Link ${idx + 1}`)}</span>
                ))}
              </div>
            );
          } else if (typeof value === "string" && isPotentialUrl(value)) {
          ...
          return (
            <div key={key} className="flex items-center py-3 border-b border-gray-100 dark:border-gray-600 last:border-b-0">
              <span className="text-gray-600 dark:text-gray-400 font-semibold w-auto flex-shrink-0 mr-3">{displayKey}:</span>
              <span className="text-gray-900 dark:text-gray-100 font-medium">{displayValue}</span>
            </div>
          );
        })}
      </div>
              ...
              <tr className="border-b border-gray-200 dark:border-gray-600">
                {columns.map(column => {
                  ...
                  return (
                    <th 
                      key={column} 
                      className={`text-left p-2 font-semibold text-gray-700 dark:text-gray-300 ${
                        column.toLowerCase().includes('email') ? 'w-48' : 
                        column === 'recording_link' ? 'w-20' : 
                        column.toLowerCase().includes('date') ? 'w-24' : 
                        'w-auto'
    ...
    if (!records || records.length === 0) {
      return (
        <div className="bg-white dark:bg-gray-800 rounded-lg border border-gray-200 dark:border-gray-700 p-4">
          ...
          <p className="text-gray-500 dark:text-gray-400 text-sm">No session records found</p>
        </div>
      );
    }
