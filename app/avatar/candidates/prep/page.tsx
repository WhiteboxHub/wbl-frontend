...
export default function CandidatesPrepPage() {
  ...
  const handleAddCandidate = async () => {
    try {
      if (!newCandidate.candidate_id || !newCandidate.batch || !newCandidate.start_date || !newCandidate.status) {
        alert("Please fill all required fields.");
        return;
      }

      const payload = {
        candidate: Number(newCandidate.candidate_id),
        batch: newCandidate.batch,
        start_date: newCandidate.start_date,
        status: newCandidate.status,
        instructor1: newCandidate.instructor1_id ? Number(newCandidate.instructor1_id) : null,
        instructor2: newCandidate.instructor2_id ? Number(newCandidate.instructor2_id) : null,
        instructor3: newCandidate.instructor3_id ? Number(newCandidate.instructor3_id) : null,
      };

      const res = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/candidate_preparation`, payload);
      ...
      {showAddForm && (
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-40 z-50">
          <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-lg w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="space-y-3">
              <div className="space-y-1">
                <Label className="text-sm font-medium text-gray-700 dark:text-gray-300">Candidate ID</Label>
                <Input
                ...
                <div key={num} className="space-y-1">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      setNewCandidate({
                        ...newCandidate,
                        [`instructor${num}_id`]: Number(e.target.value),
                      });
                    }}
                    className="w-full p-2 border rounded"
                  >
