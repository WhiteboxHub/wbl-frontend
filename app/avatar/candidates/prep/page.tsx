...
//   );
// }
// app/avatar/candidates/prep/page.tsx (or wherever you keep it)
// "use client";
"use client";
import Link from "next/link";
import "@/styles/admin.css";
...
import { createPortal } from "react-dom";
import { toast, Toaster } from "sonner";
import api from "@/lib/api"; // <<-- uses the api helper you created
...
export default function CandidatesPrepPage() {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        setLoading(true);
        // api.get returns { data: body } from the helper
        const [candidatesRes, instructorsRes] = await Promise.all([
  ...
  useEffect(() => {
    ...
    // Normalize selectedStatuses to lowercase for comparisons
    ...
    // Sort Active on top, then by id desc (defensive guards)
    filtered.sort((a, b) => {
  ...
  const CandidateNameRenderer = (params: any) => {
    // support nested candidate.full_name or candidate_name fields
    const candidateId =
      params?.data?.candidate_id || params?.data?.candidate?.id || params?.data?.id;
    const candidateName =
  ...
  const handleRowUpdated = async (updatedRow: any) => {
    try {
      const id = updatedRow?.id ?? updatedRow?.candidate_id;
      if (!id) throw new Error("Missing id on updated row");
      // the api helper will attach token header automatically if configured there
      await api.put(`/candidate_preparation/${id}`, updatedRow);
      setFilteredCandidates((prev) => prev.map((row) => (row.id === id ? { ...row, ...updatedRow } : row)));
      toast.success("Candidate preparation updated successfully!");
    } catch (err) {
      console.error("Failed to update:", err);
      toast.error("Failed to update candidate preparation.");
  ...
  const handleRowDeleted = async (id: number | string) => {
    try {
      ...
      setFilteredCandidates((prev) => prev.filter((row) => (row.id ?? row.candidate_id) !== id));
      toast.success("Candidate preparation deleted successfully!");
    } catch (err) {
      console.error("Failed to delete:", err);
      toast.error("Failed to delete candidate preparation.");
