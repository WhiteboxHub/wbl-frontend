...
export default function PlacementFeesPage() {
  ...
  const fetchFees = async () => {
    ...
    try {
      ...
      const cols: ColDef[] = [
        ...
        { field: "placement_id", headerName: "Placement ID", width: 130, editable: true },
        
        { field: "installment_id", headerName: "Installment ID", width: 130, editable: true },

        { field: "deposit_date", headerName: "Deposit Date", minWidth: 140, editable: true, cellRenderer: DateRenderer },

        { field: "deposit_amount", headerName: "Deposit Amount", minWidth: 150, editable: true, cellRenderer: AmountRenderer },
        ...
        { field: "lastmod_user_id", headerName: "Last Modified User", width: 160, editable: true },

        { field: "notes", headerName: "Notes", minWidth: 200, editable: true }
      ];

      setColumnDefs(cols);

    } catch (err: any) {
      const msg = err.response?.data?.detail || "Failed to fetch fee collections";
      setError(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  const handleRowUpdated = async (updatedRow: PlacementFee) => {
    try {
      await api.put(`/candidate/placement_fees/${updatedRow.id}`, updatedRow);
      setAllFees(prev => prev.map(f => f.id === updatedRow.id ? updatedRow : f));
  ...
  return (
    <div className="space-y-6 p-4 md:p-6">
      ...
      {/* Header */}
      <h1 className="text-2xl font-bold">Placement Fee Collections</h1>
