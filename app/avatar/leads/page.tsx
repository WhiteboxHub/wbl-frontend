...
export default function LeadsPage() {
  ...
  // Custom input handlers for React Hook Form
  const handlePhoneChange = (e: React.ChangeEvent<HTMLInputElement>, fieldName: 'phone' | 'secondary_phone') => {
  ...
  const handleNameChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    setValue('full_name', sanitizedValue);
  };

  const handleAddressChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    const sanitizedValue = e.target.value.replace(/[^a-zA-Z0-9, ]/g, "");
    setValue('address', sanitizedValue);
  };

  // Updated form submission with React Hook Form
  const handleNewLeadFormSubmit = async (data: FormData) => {
    setFormSaveLoading(true);
    try {
      const updatedData = { ...data };
      
      // Preserve existing logic for default values
      if (!updatedData.status || updatedData.status === "") {
        updatedData.status = "waiting for status";
      }
      if (!updatedData.workstatus || updatedData.workstatus === "") {
      ...
      toast.success("Lead created successfully!");
      setNewLeadForm(false);
      reset(initialFormData); // Reset form using react-hook-form
      fetchLeads(searchTerm, searchBy, sortModel);
    } catch (error) {
      toast.error("Failed to create lead");
      console.error("Error creating lead:", error);
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 p-4">
    <div className="relative min-h-[40vh] w-full max-w-xl rounded-lg bg-white p-4 shadow-md">
      ...
      <form onSubmit={handleSubmit(handleNewLeadFormSubmit)} className="grid grid-cols-1 gap-2 md:grid-cols-2">
        {/* Full Name */}
        <div>
          <input
            type="text"
            ...
            className="w-full rounded-sm border border-gray-300 px-2 py-1 text-sm focus:outline-none focus:ring-1 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700 placeholder-gray-800 dark:placeholder-gray-200"
          />
          {errors.full_name && (
...
"
        ...
        {/* Work Status and Status - Side by Side in single row */}
        <div className="md:col-span-2">
          <div className="grid grid-cols-1 gap-2 md:grid-cols-2">
              ...
              >
                {workStatusOptions.map((option) => (
                  <option
                    key={option}
                    value={option}
                    style={getWorkStatusColor(option)}
                  >
                    {option}
                  </option>
                ))}
            ...
            <div>
              ...
              <select
                ...
                {...register("status")}
                className="w-full rounded-md border border-gray-300 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-600 dark:bg-gray-700"
                style={getStatusColor(formData.status)}
              >
                {statusOptions.map((option) => (
                  <option
                    key={option}
                    value={option}
                    style={getStatusColor(option)}
                  >
                    {option}
                  </option>
                ))}
