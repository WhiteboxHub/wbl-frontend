  ...
  useEffect(() => {
    ...
    async function initSession() {
      try {
        ...
        // Fetch assessment details and assigned questions directly from backend API
        const details = await aiprepApi.getAssessment(Number(assessmentId));
        const finalType: AssessmentType = (details?.assessment_type as AssessmentType) || 'INTRO';
        const finalMode: MediaType = (details?.media_type as MediaType) || 'VIDEO';

        setAssessmentType(finalType);
        setMediaType(finalMode);

        // Retrieve questions directly from database via backend assessment details
        let backendQuestions: any[] = details?.questions || (details?.data as any)?.questions || [];

        // Fallback: If details didn't contain questions, query Question Bank directly from database API
        if (!backendQuestions || backendQuestions.length === 0) {
          try {
            const bankResponse = await aiprepApi.getQuestions(finalType);
            const bankItems = Array.isArray(bankResponse)
              ? bankResponse
              : bankResponse?.items || bankResponse?.questions || [];
            if (bankItems && bankItems.length > 0) {
              backendQuestions = bankItems;
            }
          } catch (qErr) {
            console.warn('Backend DB question bank fetch failed:', qErr);
          }
        }

        // Map and normalize questions from database
        const resolvedQuestions: QuestionBankItem[] = Array.isArray(backendQuestions)
          ? backendQuestions
              .filter((q: any) => q && (q.question_text || q.text || q.question))
              .map((q: any, idx: number) => ({
                id: q.id || q.question_id || idx + 1,
                category: (q.category as any) || finalType,
        ...
        if (resolvedQuestions.length === 0) {
          setErrorMsg('No questions found in the database for this assessment track.');
        }
  ...
  const handleEndSession = async () => {
    ...
    try {
      ...
      const telemetryPayload = {
        ...
        audio_telemetry: {
          words_per_minute: calculatedWpm || 0,
          silence_ratio_pct: 0,
          speaking_duration_seconds: Math.max(elapsedTime, 0),
        },
        video_telemetry: {
      ...
      <div className="flex-1 min-h-0 p-3 sm:p-4 grid grid-cols-1 lg:grid-cols-12 gap-3 lg:gap-4 max-w-[1600px] mx-auto w-full">
        ...
        <div className="lg:col-span-7 flex flex-col justify-between min-h-0 gap-3">
          ...
          <div className="relative w-full aspect-video sm:aspect-auto sm:flex-1 rounded-2xl sm:rounded-3xl overflow-hidden bg-slate-950 border border-slate-200 dark:border-slate-800/80 shadow-lg dark:shadow-2xl flex items-center justify-center min-h-[260px] sm:min-h-[360px]">
            {/* 5-4-3-2-1 Countdown Overlay for Intro Track */}
            {countdownValue !== null && (
            ...
            <div className="flex items-center justify-end gap-2 pt-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const targetDashboardUrl = isEmbedded ? '/user_dashboard/ai-prep?embed=true' : '/user_dashboard/ai-prep';
                  if (typeof window !== 'undefined' && window.top && window.top !== window.self) {
                    window.top.location.href = targetDashboardUrl;
                  } else {
