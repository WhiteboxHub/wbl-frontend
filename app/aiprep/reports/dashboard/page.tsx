...
function ReportsDashboardContent() {
  ...
  const view = searchParams.get('view') || 'assessments';
  const [, setCandidateId] = useState<number | null>(null);
  ...
  return (
    <div className="min-h-screen bg-slate-50/50 px-4 pb-8 pt-24 sm:px-8 dark:bg-slate-950">
      <div className="mx-auto max-w-6xl space-y-6">
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <button
              onClick={() => router.push('/user_dashboard/ai-prep')}
              className="rounded-xl border border-slate-200 bg-white p-2.5 text-slate-700 shadow-xs hover:bg-slate-50 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-200 cursor-pointer"
            >
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-slate-400">Reporting Suite</span>
              </div>
              <h1 className="text-2xl font-black text-slate-900 dark:text-white sm:text-3xl">
                {view === 'analytics' ? 'Assessment Analytics' : view === 'scores' ? 'Assessment Scores' : 'Evaluation Reports'}
              </h1>
            </div>
          </div>
          ...
          <button
            onClick={() => router.push('/user_dashboard/ai-prep')}
            className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2.5 text-xs font-bold text-white shadow-xs hover:bg-indigo-500 transition-all cursor-pointer"
          >
          ...
          <div className="flex items-center justify-between border-b border-slate-100 pb-4 dark:border-slate-800">
            <div>
              <h2 className="text-lg font-bold text-slate-900 dark:text-white">
                {view === 'analytics' ? 'Candidate Progress Grid' : view === 'scores' ? 'Scorecard Grid' : 'Past Evaluation Sessions'}
              </h2>
          ...
          {loading ? (
            ...
            <div className="py-16 text-center text-slate-400">
              ...
              <p className="mt-1 text-xs text-slate-400 max-w-sm mx-auto">
                Start your first practice session in AI Prep to generate a comprehensive evaluation report.
              </p>
              <button
                onClick={() => router.push('/user_dashboard/ai-prep')}
                className="mt-4 inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-xs font-bold text-white shadow-xs hover:bg-indigo-500 cursor-pointer"
              >
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              {completedSessions.map((session) => (
                <div
                  key={session.id}
                  className="rounded-2xl border border-slate-200 bg-slate-50/50 p-4 transition hover:border-indigo-300 hover:bg-indigo-50/30 dark:border-slate-800 dark:bg-slate-800/20"
                >
                  ...
                  <div className="mt-4 flex items-center justify-between gap-3">
                    <span
                      className={`rounded-full px-2.5 py-1 text-xs font-bold ${session.status === 'COMPLETED'
                    ...
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3.5 py-1.5 text-xs font-bold text-white shadow-xs hover:bg-indigo-500 transition-all cursor-pointer"
                    >
