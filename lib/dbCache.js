...
// Delete ALL cached entries whose key starts with the given prefix.
// e.g. deleteByPrefix("/leads") removes "/leads", "/leads?sort=entry_date:desc", etc.
export async function deleteByPrefix(prefix) {
  try {
    ...
    const normalizedPrefix = prefix.replace(/^\//, "").split("?")[0];
    ...
    db.close();
    logger.log(`[Cache] Invalidated all entries with prefix: ${prefix}`);
  } catch (error) {
    logger.error("[Cache] deleteByPrefix error", error);
  }
}
