...
// Short-term in-memory cache for version checks to prevent redundant HEAD requests in loops
...
async function checkCacheFreshness(endpoint, cachedLastMod) {
  try {
    ...
    // Check in-memory version cache first
    const now = Date.now();
    const cachedVersion = versionCache.get(url);
    if (cachedVersion && (now - cachedVersion.timestamp < VERSION_CACHE_TTL)) {
      return cachedVersion.version === cachedLastMod;
    }
    ...
    // Store in version cache
    if (serverLastMod) {
      versionCache.set(url, {
        version: serverLastMod,
        timestamp: now
      });
    }
    ...
    if (serverNormalized !== cachedNormalized) {
      ...
      for (let i = 0; i < maxLen; i++) {
        ...
        if (sChar !== cChar) {
          logger.log(`[Cache] Diff at position ${i}: server='${sChar}' cached='${cChar}'`);
        }
      }
    }
    ...
    logger.error("[Cache] HEAD check error", error);
    return false;
  }
}
...
export async function invalidateCache(endpoint) {
  ...
  // Clear from in-memory version cache too
  const baseUrl = (process.env.NEXT_PUBLIC_API_URL || "").replace(/\/$/, "");
  let ep = basePath.replace(/^\//, "");
  if (ep.startsWith("api/")) ep = ep.substring(4);
  const url = `${baseUrl ? baseUrl : ''}/${ep}`;
