...
//   // Remove leading slash and any /api/ prefix from endpoint
...
//     endpointStr = endpointStr.substring(4); // Remove "api/" prefix
//   }
...
//     console.log("[apiFetch] endpoint input:", endpoint);
//     console.log("[apiFetch] endpointStr:", endpointStr);
...
// Universal updater
export async function smartUpdate(resource, id, data, opts = {}) {
  // Try the most common pattern first for leads
  const rawResource = String(resource).replace(/^\/+/, "").replace(/\/+$/, "");
  
  // Priority order: try most likely to succeed first
  const priorityCandidates = [
    { method: "PUT", endpoint: `api/leads/${id}` },
    { method: "PATCH", endpoint: `api/leads/${id}` },
    { method: "PUT", endpoint: `leads/${id}` },
    { method: "PATCH", endpoint: `leads/${id}` },
  ];
