...
async function runReview() {
  ...
  finalContext += "# 1. Code Context (Source of Truth)\n";
  finalContext += "## Git Diff\n```diff\n" + diffText + "\n```\n";
  finalContext += analysis.contextParts.join("\n") + "\n\n";
  
  finalContext += "# 2. AST Findings (Facts)\n";
  if (analysis.allFindings.length === 0) {
    finalContext += "None.\n\n";
  } else {
    finalContext += "```json\n" + JSON.stringify(analysis.allFindings, null, 2) + "\n```\n\n";
  }

  finalContext += "# 3. Downstream Impact Analysis\n";
  ...
  finalContext += "# 4. Modified Public APIs\n";
